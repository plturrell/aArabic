...
pub fn calcLoadCommandsSizeDsym(macho_file: *MachO, dsym: *const DebugSymbols) u32 {
    ...
    // LC_SEGMENT_64
    const linkedit_seg_index = macho_file.linkedit_seg_index.?;
    sizeofcmds += @sizeOf(macho.segment_command_64) * @as(u64, linkedit_seg_index);
    for (macho_file.segments.items[0..linkedit_seg_index]) |seg| {
        sizeofcmds += @as(u64, seg.nsects) * @sizeOf(macho.section_64);
    }
