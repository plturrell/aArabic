...
fn getSegmentRank(segname: []const u8) u8 {
    ...
    if (mem.startsWith(u8, segname, "__TEXT_ZIG")) return 0xe;
    if (mem.startsWith(u8, segname, "__CONST_ZIG")) return 0xe;
    if (mem.startsWith(u8, segname, "__DATA_ZIG")) return 0xe;
...
fn zigSegmentIndex(name: []const u8) ?u8 {
    if (mem.startsWith(u8, name, "__TEXT_ZIG")) return 0;
    if (mem.startsWith(u8, name, "__CONST_ZIG")) return 1;
    if (mem.startsWith(u8, name, "__DATA_ZIG")) return 2;
    if (mem.startsWith(u8, name, "__BSS_ZIG")) return 3;
    return null;
}
...
fn segmentLessThan(ctx: void, lhs: []const u8, rhs: []const u8) bool {
    ...
    if (lhs_rank == rhs_rank) {
        if (lhs_rank == 0xe) {
            const l_idx = zigSegmentIndex(lhs);
            const r_idx = zigSegmentIndex(rhs);
            if (l_idx != null and r_idx != null and l_idx.? != r_idx.?) {
                return l_idx.? < r_idx.?;
            }
        }
