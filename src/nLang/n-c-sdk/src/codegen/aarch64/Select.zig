    ...
    air_tag: switch (air.next().?) {
            ...
            switch (arg_vi.parent(isel)) {
                .unallocated, .stack_slot => if (arg_vi.hint(isel)) |arg_ra| {
                    ...
                    while (arg_part_it.next()) |arg_part| {
                        const part_hint = arg_part.hint(isel) orelse
                            return isel.fail("arg part missing hint for {d}", .{@intFromEnum(arg_part)});
                        try arg_part.defLiveIn(isel, part_hint, comptime &.initFill(.free));
                    }
                },
                .value, .constant => unreachable,
                .address => |address_vi| {
                    const addr_hint = address_vi.hint(isel) orelse
                        return isel.fail("arg address missing hint for {d}", .{@intFromEnum(address_vi)});
                    try address_vi.defLiveIn(isel, addr_hint, comptime &.initFill(.free));
                },
            }
                ...
                if (!ty.isRuntimeFloat()) {
                    ...
                    switch (int_info.bits) {
                        ...
                        65...128 => |bits| {
                            ...
                            const res_hi64_vi = try res_hi64_it.only(isel) orelse break :unused;
                            const res_hi64_ra = try res_hi64_vi.defReg(isel);
                            var res_lo64_it = res_vi.value.field(ty, 0, 8);
                            const res_lo64_vi = try res_lo64_it.only(isel) orelse break :unused;
                            ...
                            const lhs_lo64_mat, const rhs_lo64_mat = lo64_mat: {
                                ...
                                var rhs_lo64_it = rhs_vi.field(ty, 0, 8);
                                const rhs_lo64_vi = try rhs_lo64_it.only(isel) orelse break :unused;
                                const rhs_lo64_mat = try rhs_lo64_vi.matReg(isel);
                                var lhs_lo64_it = lhs_vi.field(ty, 0, 8);
                                const lhs_lo64_vi = try lhs_lo64_it.only(isel) orelse break :unused;
                            ...
                            if (res_hi64_ra) |res_ra| {
                                ...
                                const rhs_hi64_vi = try rhs_hi64_it.only(isel) orelse break :unused;
                                const rhs_hi64_mat = try rhs_hi64_vi.matReg(isel);
                                var lhs_hi64_it = lhs_vi.field(ty, 8, 8);
                                const lhs_hi64_vi = try lhs_hi64_it.only(isel) orelse break :unused;
                ...
                if (!ty.isRuntimeFloat()) {
                    ...
                    switch (int_info.bits) {
                        ...
                        65...128 => {
                            ...
                            try call.prepareReturn(isel);
                            var res_hi64_it = res_vi.value.field(ty, 8, 8);
                            if (try res_hi64_it.only(isel)) |res_hi64_vi| {
                            ...
                            const rhs_vi = try isel.use(bin_op.rhs);
                            var rhs_hi64_it = rhs_vi.field(ty, 8, 8);
                            if (try rhs_hi64_it.only(isel)) |rhs_hi64_vi| {
                            ...
                            var lhs_hi64_it = lhs_vi.field(ty, 8, 8);
                            if (try lhs_hi64_it.only(isel)) |lhs_hi64_vi| {
        ...
        .ptr_add, .ptr_sub => |air_tag| {
            if (isel.live_values.fetchRemove(air.inst_index)) |res_vi| unused: {
                ...
                if (try base_part_it.only(isel)) |base_part_vi| {
        ...
        .add_with_overflow, .sub_with_overflow => |air_tag| {
            if (isel.live_values.fetchRemove(air.inst_index)) |res_vi| {
                ...
                if (try wrapped_it.only(isel)) |wrapped_vi| {
                    const overflow_opt: Value.Index.AddOrSubtractOptions.Overflow = blk: {
                        if (overflow_vi) |ov_vi| {
                            if (try ov_vi.defReg(isel)) |overflow_ra| break :blk .{ .ra = overflow_ra };
                        }
                        break :blk .wrap;
                    };
        ...
        .not => |air_tag| {
            if (isel.live_values.fetchRemove(air.inst_index)) |res_vi| {
                ...
                while (offset > 0) {
                    ...
                    const src_part_vi = try src_part_it.only(isel) orelse continue;
                ...
                if (dst_ty.isAbiInt(zcu) and (src_tag == .bool or src_ty.isAbiInt(zcu))) {
                    ...
                    if (dst_tag != .@"struct" and src_tag != .@"struct" and src_tag != .bool and dst_int_info.signedness == src_int_info.signedness) {
                        ...
                        65...127 => |dst_bits| {
                            ...
                            if (try dst_hi64_it.only(isel)) |dst_hi64_vi| {
                                if (try dst_hi64_vi.defReg(isel)) |dst_hi64_ra| {
                                    var src_hi64_it = src_vi.field(src_ty, 8, 8);
                                    const src_hi64_vi = try src_hi64_it.only(isel) orelse unreachable;
                            ...
                            if (try dst_lo64_it.only(isel)) |dst_lo64_vi| {
                                if (try dst_lo64_vi.defReg(isel)) |dst_lo64_ra| {
                                    var src_lo64_it = src_vi.field(src_ty, 0, 8);
                                    const src_lo64_vi = try src_lo64_it.only(isel) orelse unreachable;
                    ...
                    switch (dst_int_info.bits) {
                        ...
                        80 => switch (dst_int_info.signedness) {
                            .signed => {
                                ...
                                if (try dst_hi16_it.only(isel)) |dst_hi16_vi| {
                                    if (try dst_hi16_vi.defReg(isel)) |dst_hi16_ra| {
                                        var src_hi16_it = src_vi.field(src_ty, 8, 8);
                                        const src_hi16_vi = try src_hi16_it.only(isel) orelse unreachable;
        ...
        .call => {
            ...
            var maybe_def_ret_value: ?Value.Index = null;
            if (maybe_def_ret_vi) |def_ret_vi| {
                defer def_ret_vi.value.deref(isel);

                var ret_it: CallAbiIterator = .init;
                if (try ret_it.ret(isel, isel.air.typeOfIndex(air.inst_index, ip))) |ret_vi| {
                    ...
                    switch (ret_vi.parent(isel)) {
                        .unallocated, .stack_slot => if (ret_vi.hint(isel)) |ret_ra| {
                            ...
                            while (def_ret_part_it.next()) |ret_part_vi| {
                                const ret_part = ret_part_it.next() orelse break;
                                const ret_hint = ret_part.hint(isel) orelse continue;
                                try call.returnLiveIn(isel, ret_part_vi, ret_hint);
                            }
                        },
                        .value, .constant => unreachable,
                        .address => |address_vi| {
                            maybe_ret_addr_vi = address_vi;
                            maybe_def_ret_value = def_ret_vi.value;
            ...
            try call.prepareParams(isel);
            if (maybe_ret_addr_vi) |ret_addr_vi| {
                if (maybe_def_ret_value) |def_ret_value| {
                    if (ret_addr_vi.hint(isel)) |hint| {
                        try call.paramAddress(isel, def_ret_value, hint);
                    }
                }
            }
            var param_it: CallAbiIterator = .init;
                ...
                switch (param_vi.parent(isel)) {
                    .unallocated => if (param_vi.hint(isel)) |param_ra| {
                        ...
                        while (param_part_it.next()) |param_part_vi| {
                            ...
                            const param_hint = param_part_vi.hint(isel) orelse
                                return isel.fail("paramLiveOut: missing hint for part {d}", .{@intFromEnum(param_part_vi)});
                            try call.paramLiveOut(isel, arg_part_vi, param_hint);
                        }
                    },
                    ...
                    .address => |address_vi| {
                        const addr_hint = address_vi.hint(isel) orelse
                            return isel.fail("paramAddress: missing hint for address value {d}", .{@intFromEnum(address_vi)});
                        try call.paramAddress(isel, arg_vi, addr_hint);
                    },
                }
            }
            try call.finishParams(isel);
            ...
            if (isel.live_values.fetchRemove(air.inst_index)) |res_vi| unused: {
                ...
                switch (int_info.bits) {
                    ...
                    65...128 => |bits| {
                        ...
                        const src_hi64_vi = try src_hi64_it.only(isel) orelse break :unused;
                        const src_hi64_mat = try src_hi64_vi.matReg(isel);
                        var src_lo64_it = src_vi.field(ty, 0, 8);
                        const src_lo64_vi = try src_lo64_it.only(isel) orelse break :unused;
                        const src_lo64_mat = try src_lo64_vi.matReg(isel);
                ...
                switch (ty.floatBits(isel.target)) {
                    ...
                    80, 128 => |bits| {
                        ...
                        switch (bits) {
                            ...
                            80 => {
                                var res_hi16_it = res_vi.value.field(ty, 8, 8);
                                const res_hi16_vi = try res_hi16_it.only(isel) orelse unreachable;
                                try call.returnLiveIn(isel, res_hi16_vi, .r1);
                                var res_lo64_it = res_vi.value.field(ty, 0, 8);
                                const res_lo64_vi = try res_lo64_it.only(isel) orelse unreachable;
                                try call.returnLiveIn(isel, res_lo64_vi, .r0);
                            },
                        }
                        try call.finishReturn(isel);
                ...
                if (!ty.isRuntimeFloat()) {
                    ...
                    80, 128 => |bits| {
                        ...
                        switch (bits) {
                            ...
                            80 => {
                                var rhs_hi16_it = rhs_vi.field(ty, 8, 8);
                                const rhs_hi16_vi = try rhs_hi16_it.only(isel) orelse return error.CodegenFail;
                                try call.paramLiveOut(isel, rhs_hi16_vi, .r3);
                                var rhs_lo64_it = rhs_vi.field(ty, 0, 8);
                                const rhs_lo64_vi = try rhs_lo64_it.only(isel) orelse return error.CodegenFail;
                                try call.paramLiveOut(isel, rhs_lo64_vi, .r2);
                                var lhs_hi16_it = lhs_vi.field(ty, 8, 8);
                                const lhs_hi16_vi = try lhs_hi16_it.only(isel) orelse return error.CodegenFail;
                                try call.paramLiveOut(isel, lhs_hi16_vi, .r1);
                                var lhs_lo64_it = lhs_vi.field(ty, 0, 8);
                                const lhs_lo64_vi = try lhs_lo64_it.only(isel) orelse return error.CodegenFail;
                                try call.paramLiveOut(isel, lhs_lo64_vi, .r0);
                            },
                        }
                        try call.finishParams(isel);
        ...
        .@"try", .try_cold => {
            ...
            if (isel.live_values.fetchRemove(air.inst_index)) |payload_vi| {
                ...
                if (try payload_part_it.only(isel)) |payload_part_vi| {
            ...
            if (try error_set_part_it.only(isel)) |error_set_part_vi| {
        ...
        .is_null, .is_non_null => |air_tag| {
            if (isel.live_values.fetchRemove(air.inst_index)) |is_vi| unused: {
                ...
                if (try has_value_part_it.only(isel)) |has_value_part_vi| {
            ...
            if (isel.live_values.get(Block.main)) |ret_vi| {
                ...
                switch (ret_vi.parent(isel)) {
                    .unallocated, .stack_slot => if (ret_vi.hint(isel)) |ret_ra| {
                        ...
                        while (ret_part_it.next()) |ret_part_vi| {
                            const src_part_vi = src_part_it.next() orelse break;
                            ...
                            if (ret_part_vi.hint(isel)) |hint| {
                                try src_part_vi.liveOut(isel, hint);
                            }
                        }
                    },
                    .value, .constant => unreachable,
        ...
        .ret_load => {
            ...
            if (isel.live_values.get(Block.main)) |ret_vi| switch (ret_vi.parent(isel)) {
                .unallocated, .stack_slot => {
                    ...
                    while (ret_part_it.next()) |ret_part_vi| {
                        if (ret_part_vi.hint(isel)) |hint| {
                            try ret_part_vi.liveOut(isel, hint);
                        }
                    }
                ...
                switch (@max(dst_bits, src_bits)) {
                    ...
                    80, 128 => {
                        ...
                        switch (dst_bits) {
                            ...
                            80 => {
                                var dst_hi16_it = dst_vi.value.field(dst_ty, 8, 8);
                                const dst_hi16_vi = try dst_hi16_it.only(isel) orelse break :unused;
                                try call.returnLiveIn(isel, dst_hi16_vi, .r1);
                                var dst_lo64_it = dst_vi.value.field(dst_ty, 0, 8);
                                const dst_lo64_vi = try dst_lo64_it.only(isel) orelse break :unused;
                        ...
                        switch (src_bits) {
                            ...
                            80 => {
                                var src_hi16_it = src_vi.field(src_ty, 8, 8);
                                const src_hi16_vi = try src_hi16_it.only(isel) orelse break :unused;
                                try call.paramLiveOut(isel, src_hi16_vi, .r1);
                                var src_lo64_it = src_vi.field(src_ty, 0, 8);
                                const src_lo64_vi = try src_lo64_it.only(isel) orelse break :unused;
                                try call.paramLiveOut(isel, src_lo64_vi, .r0);
        ...
        .intcast => |air_tag| {
            if (isel.live_values.fetchRemove(air.inst_index)) |dst_vi| unused: {
                        ...
                        (dst_int_info.bits - 1) / 128 == (src_int_info.bits - 1) / 128))
                {
                    ...
                    var dst_hi64_it = dst_vi.value.field(dst_ty, 8, 8);
                    const dst_hi64_vi = try dst_hi64_it.only(isel) orelse break :unused;
            ...
            if (isel.live_values.fetchRemove(air.inst_index)) |dst_vi| unused: {
                ...
                switch (dst_int_info.bits) {
                    ...
                    1...64 => |dst_bits| {
                        ...
                        const src_part_vi = try src_part_it.only(isel) orelse break :unused;
                        const src_part_mat = try src_part_vi.matReg(isel);
        ...
        .wrap_errunion_payload => {
            if (isel.live_values.fetchRemove(air.inst_index)) |error_union_vi| {
                ...
                if (try error_set_part_it.only(isel)) |error_set_part_vi| if (try error_set_part_vi.defReg(isel)) |error_set_part_ra| try isel.emit(switch (error_set_size) {
                    else => unreachable,
                ...
                switch (agg_ty.zigTypeTag(zcu)) {
                    ...
                    .@"struct" => {
                        ...
                        while (try agg_part_it.next(isel)) |agg_part| {
                            ...
                            const field_part_vi = try field_part_it.only(isel) orelse continue;
                            ...
                            while (field_subpart_it.next()) |field_subpart_vi| {
                                ...
                                if (try agg_subpart_it.only(isel)) |agg_subpart_vi| {
                    ...
                    .@"union" => {
                        ...
                        if (try payload_it.only(isel)) |payload_vi| {
        ...
        .get_union_tag => {
            if (isel.live_values.fetchRemove(air.inst_index)) |tag_vi| {
                ...
                if (try tag_part_it.only(isel)) |tag_part_vi| {
        ...
        .slice_elem_val => {
            if (isel.live_values.fetchRemove(air.inst_index)) |elem_vi| unused: {
                ...
                if (switch (elem_size) {
                    ...
                    const ptr_part_vi = try ptr_part_it.only(isel) orelse break :unused;
                ...
                switch (@max(dst_int_info.bits, src_bits)) {
                    ...
                    65...128 => {
                        ...
                        switch (dst_int_info.bits) {
                            ...
                            65...128 => {
                                var dst_hi64_it = dst_vi.value.field(dst_ty, 8, 8);
                                const dst_hi64_vi = try dst_hi64_it.only(isel) orelse return error.CodegenFail;
                                try call.returnLiveIn(isel, dst_hi64_vi, .r1);
                                var dst_lo64_it = dst_vi.value.field(dst_ty, 0, 8);
                                const dst_lo64_vi = try dst_lo64_it.only(isel) orelse return error.CodegenFail;
                                try call.returnLiveIn(isel, dst_lo64_vi, .r0);
                            },
                        }
                        try call.finishReturn(isel);
                        ...
                        switch (src_bits) {
                            ...
                            80 => {
                                var src_hi16_it = src_vi.field(src_ty, 8, 8);
                                const src_hi16_vi = try src_hi16_it.only(isel) orelse return error.CodegenFail;
                                try call.paramLiveOut(isel, src_hi16_vi, .r1);
                                var src_lo64_it = src_vi.field(src_ty, 0, 8);
                                const src_lo64_vi = try src_lo64_it.only(isel) orelse return error.CodegenFail;
                                try call.paramLiveOut(isel, src_lo64_vi, .r0);
                            },
                        }
                        try call.finishParams(isel);
                ...
                switch (@max(dst_bits, src_int_info.bits)) {
                    ...
                    65...128 => {
                        ...
                        switch (dst_bits) {
                            ...
                            80 => {
                                var dst_hi16_it = dst_vi.value.field(dst_ty, 8, 8);
                                const dst_hi16_vi = try dst_hi16_it.only(isel) orelse return error.CodegenFail;
                                try call.returnLiveIn(isel, dst_hi16_vi, .r1);
                                var dst_lo64_it = dst_vi.value.field(dst_ty, 0, 8);
                                const dst_lo64_vi = try dst_lo64_it.only(isel) orelse return error.CodegenFail;
                                try call.returnLiveIn(isel, dst_lo64_vi, .r0);
                        ...
                        switch (src_int_info.bits) {
                            ...
                            65...128 => {
                                var src_hi64_it = src_vi.field(src_ty, 8, 8);
                                const src_hi64_vi = try src_hi64_it.only(isel) orelse return error.CodegenFail;
        ...
        .memset, .memset_safe => |air_tag| {
                    ...
                    2, 4, 8 => |size| {
                        ...
                        const len_mat: Value.Materialize = len_mat: switch (dst_info.flags.size) {
                            ...
                            .slice => {
                                ...
                                const dst_len_vi = try dst_len_it.only(isel) orelse break :len_mat .{ .vi = undefined, .ra = undefined };
                        ...
                        switch (dst_info.flags.size) {
                            ...
                            .slice => {
                                ...
                                const dst_ptr_vi = try dst_ptr_it.only(isel) orelse return;
            ...
            switch (dst_info.flags.size) {
                ...
                .slice => {
                    var dst_ptr_it = dst_vi.field(dst_ty, 0, 8);
                    if (try dst_ptr_it.only(isel)) |dst_ptr_vi| {
                        var dst_len_it = dst_vi.field(dst_ty, 8, 8);
                        if (try dst_len_it.only(isel)) |dst_len_vi| {
        ...
        .error_name => {
            if (isel.live_values.fetchRemove(air.inst_index)) |name_vi| unused: {
                ...
                const len_part_vi = try len_part_it.only(isel) orelse break :unused;
                ...
                switch (ip.indexToKey(agg_ty.toIntern())) {
                    .array_type => |array_type| {
                        ...
                        for (elems) |elem| {
                            ...
                            if (try agg_part_it.only(isel)) |agg_part_vi| {
        ...
        .union_init => |air_tag| {
            if (isel.live_values.fetchRemove(air.inst_index)) |union_vi| unused: {
                ...
                if (union_layout.tag_size > 0) unused_tag: {
                    ...
                    const tag_vi = try tag_it.only(isel) orelse break :unused_tag;
                ...
                const payload_vi = try payload_it.only(isel) orelse break :unused;
        ...
        .prefetch => {
            ...
            if (!(prefetch.rw == .write and prefetch.cache == .instruction)) {
                ...
                const ptr_vi = if (maybe_slice_ty.isSlice(zcu)) ptr_vi: {
                    ...
                    const ptr_part_vi = try ptr_part_it.only(isel) orelse break :ptr_vi maybe_slice_vi;
                ...
                switch (ty.floatBits(isel.target)) {
                    ...
                    80, 128 => |bits| {
                        ...
                        switch (bits) {
                            ...
                            80 => {
                                var res_hi16_it = res_vi.value.field(ty, 8, 8);
                                const res_hi16_vi = try res_hi16_it.only(isel) orelse return error.CodegenFail;
                                try call.returnLiveIn(isel, res_hi16_vi, .r1);
                                var res_lo64_it = res_vi.value.field(ty, 0, 8);
                                const res_lo64_vi = try res_lo64_it.only(isel) orelse return error.CodegenFail;
                                try call.returnLiveIn(isel, res_lo64_vi, .r0);
                            },
                        }
                        try call.finishReturn(isel);
                        ...
                        switch (bits) {
                            ...
                            80 => {
                                ...
                                const addend_hi16_vi = try addend_hi16_it.only(isel) orelse return error.CodegenFail;
                                try call.paramLiveOut(isel, addend_hi16_vi, .r5);
                                var addend_lo64_it = addend_vi.field(ty, 0, 8);
                                const addend_lo64_vi = try addend_lo64_it.only(isel) orelse return error.CodegenFail;
        ...
        .c_va_arg => {
            ...
            const ty = ty_op.ty.toType();
            var param_it: CallAbiIterator = .init;
            const param_vi = try param_it.param(isel, ty) orelse unreachable;
            defer param_vi.deref(isel);
            const passed_vi = switch (param_vi.parent(isel)) {
            ...
            const parts_len = parts_len: {
                ...
                while (part_it.next()) |part_vi| : (parts_len += 1) {
                    ...
                    if (try arg_part_it.only(isel)) |arg_part_vi| {
            ...
            if (isel.live_values.fetchRemove(air.inst_index)) |va_list_vi| {
                ...
                switch (isel.va_list) {
                    ...
                    .sysv => |va_list| {
                        ...
                        if (try vr_offs_it.only(isel)) |vr_offs_vi| {
                        ...
                        if (try gr_offs_it.only(isel)) |gr_offs_vi| {
                        ...
                        if (try vr_top_it.only(isel)) |vr_top_vi| {
                        ...
                        if (try gr_top_it.only(isel)) |gr_top_vi| {
                        ...
                        if (try stack_it.only(isel)) |stack_vi| {
...
pub fn verify(isel: *Select, check_values: bool) void {
    ...
    while (live_reg_it.next()) |live_reg_entry| switch (live_reg_entry.value.*) {
        .allocating, .free => {},
            ...
            isel.dumpValues(.all);
            std.debug.panic("verify: register {s} left live with value {d}", .{
                @tagName(live_reg_entry.key),
                @intFromEnum(live_reg_entry.value.*),
            });
        },
    };
    if (check_values) {
        var bad_refs = false;
        for (isel.values.items, 0..) |value, i| {
            if (value.refs == 0) continue;
            if (!bad_refs) std.debug.print("verify: leaked value refs\n", .{});
            std.debug.print("  value {d} refs={d} parent={s} loc={s}\n", .{
                i,
                value.refs,
                @tagName(value.flags.parent_tag),
                @tagName(value.flags.location_tag),
            });
            bad_refs = true;
        }
        if (bad_refs) {
            isel.dumpValues(.only_referenced);
            std.debug.panic("verify: leaked value refs", .{});
        }
    }
}
...
fn storeReg(
    ...
    switch (size) {
        ...
        16 => {
            if (!ra.isVector()) return isel.fail("storeReg: 16-byte store requires vector register", .{});
...
pub const Value = struct {
    ...
    pub const Index = enum(u32) {
        ...
        fn setParts(vi: Value.Index, isel: *Select, parts_len: Value.PartsLen) void {
            // A single part needs no partitioning; treat 0/1 as the trivial case.
        ...
        fn copyAdvanced(dst_vi: Value.Index, isel: *Select, src_vi: Value.Index, root: struct {
            ...
            // Guard against infinite recursion - max depth of 100 should be plenty for any valid type
            if (root.depth > 100) {
                return isel.fail("copyAdvanced recursion depth overflow for type {f}", .{isel.fmtType(root.ty)});
            }
            const log_once = std.process.hasEnvVar(isel.pt.zcu.gpa, "ZIG_LOG_COPY_ONCE") catch false;
            if (log_once) {
                const AtomicBool = @import("std").atomic.Value(bool);
                const Once = struct {
                    pub var hit = AtomicBool.init(false);
                };
                if (!Once.hit.swap(true, .seq_cst)) {
                    std.debug.print(
                        "COPY_ONCE: ty={f} dst_size={d} src_size={d} dst_off={d} src_off={d}\n",
                        .{ isel.fmtType(root.ty), dst_vi.size(isel), src_vi.size(isel), root.dst_offset, root.src_offset },
                    );
                }
            }
            ...
            if (dst_part_it.only()) |dst_part_vi| {
                ...
                if (src_part_it.only()) |src_part_vi| only: {
                    ...
                    if (src_part_size > @as(@TypeOf(src_part_size), if (src_part_vi.isVector(isel)) 16 else 8)) {
                        ...
                        src_part_it = src_vi.parts(isel);
                        // If splitting failed to materialize multiple parts, just fall back to the
                        // multi-part path below without asserting.
                        if (src_part_it.only() != null) break :only;
                        break :only;
                    }
                    return src_part_vi.liveOut(isel, try dst_part_vi.defReg(isel) orelse return);
                }
                var guard_src_iters: u32 = 0;
                while (src_part_it.next()) |src_part_vi| {
                    guard_src_iters += 1;
                    if (guard_src_iters > 100_000) {
                        return isel.fail("copyAdvanced src iteration overflow for type {f}", .{isel.fmtType(root.ty)});
                    }
                    ...
                    if (try dst_field_it.only(isel)) |dst_field_vi| {
                ...
                var guard_dst_iters: u32 = 0;
                while (dst_part_it.next()) |dst_part_vi| {
                    guard_dst_iters += 1;
                    if (guard_dst_iters > 100_000) {
                        return isel.fail("copyAdvanced dst iteration overflow for type {f}", .{isel.fmtType(root.ty)});
                    }
                    ...
                    if (try src_field_it.only(isel)) |src_part_vi| {
        ...
        fn addOrSubtract(
            ...
            while (part_offset > 0) : (need_wrap = false) {
                ...
                const wrapped_res_part_vi = try wrapped_res_part_it.only(isel) orelse if (need_carry) continue else continue;
            ...
            if (part_it.only()) |part_vi| only: {
                ...
                if (part_size > @as(@TypeOf(part_size), if (part_is_vector) 16 else 8)) {
                    ...
                    if (part_it.only() != null) {
                        return isel.fail("load: failed to split part (type {f})", .{isel.fmtType(root_ty)});
                    }
                    break :only;
                }
        ...
        fn store(
            ...
            if (part_it.only()) |part_vi| only: {
                ...
                if (part_size > @as(@TypeOf(part_size), if (part_is_vector) 16 else 8)) {
                    ...
                    const chunk: u64 = if (part_is_vector) 16 else 8;
                    const needed_parts = std.math.divCeil(u64, part_size, chunk) catch unreachable;
                    if (needed_parts > Value.max_parts) return isel.fail(
                        "store: unable to split {d}-byte value into <= {d} parts",
                        .{ part_size, Value.max_parts },
                    );
                    try isel.values.ensureUnusedCapacity(zcu.gpa, @intCast(needed_parts));
                    part_vi.setParts(isel, @intCast(needed_parts));
                    var remain = part_size;
                    var local_off: u64 = 0;
                    for (0..needed_parts) |_| {
                        const this_size = @min(remain, chunk);
                        _ = part_vi.addPart(isel, local_off, this_size);
                        local_off += this_size;
                        remain -= this_size;
                    }
                    part_it = vi.parts(isel);
                        ...
                        while (part_it.next()) |part_vi| {
                            ...
                            try isel.emit(if (part_vi.isVector(isel)) emit: {
                                ...
                                1...4 => .bfm(ra.w(), part_mat.ra.w(), .{
                                    ...
                                    .imms = blk: {
                                        if (std.math.cast(u5, 8 * part_size - 1)) |imms| {
                                            break :blk imms;
                                        } else {
                                            return isel.fail("defReg: imms overflow (part_size {d})", .{part_size});
                                        }
                                    },
                                }),
                                5...8 => .bfm(ra.x(), part_mat.ra.x(), .{
                                    ...
                                    .imms = blk: {
                                        if (std.math.cast(u6, 8 * part_size - 1)) |imms| {
                                            break :blk imms;
                                        } else {
                                            return isel.fail("defReg: imms overflow (part_size {d})", .{part_size});
                                        }
        ...
        fn spillReg(
            ...
            if (part_it.only()) |part_vi| {
                ...
                const end_offset = std.math.add(u64, start_offset, part_size) catch {
                    return isel.fail("spillReg: end_offset overflow (start {d}, size {d})", .{ start_offset, part_size });
                };
                try isel.emit(switch (end_offset) {
                    else => return isel.fail("spillReg: unexpected end_offset {d} (size {d})", .{ end_offset, part_size }),
                    1...4 => switch (part_vi.signedness(isel)) {
                        .signed => .sbfm(part_ra.w(), src_ra.w(), .{
                            .N = .word,
                            .immr = (std.math.cast(u5, 8 * start_offset) orelse
                                return isel.fail("spillReg: immr overflow", .{})),
                            .imms = (std.math.cast(u5, 8 * end_offset - 1) orelse
                                return isel.fail("spillReg: imms overflow", .{})),
                        }),
                    ...
                    5...8 => switch (part_vi.signedness(isel)) {
                        .signed => .sbfm(part_ra.x(), src_ra.x(), .{
                            .N = .doubleword,
                            .immr = (std.math.cast(u6, 8 * start_offset) orelse
                                return isel.fail("spillReg: immr overflow", .{})),
                            .imms = (std.math.cast(u6, 8 * end_offset - 1) orelse
                                return isel.fail("spillReg: imms overflow", .{})),
                        }),
        ...
        fn liveOut(vi: Value.Index, isel: *Select, ra: Register.Alias) !void {
            if (!(try isel.fill(ra))) {
                return isel.fail("liveOut: unable to fill register {any} for value {d}", .{ ra, @intFromEnum(vi) });
            }
            const live_vi = isel.live_registers.getPtr(ra);
            if (live_vi.* != .free) {
                return isel.fail("liveOut: register {any} not free (state {any})", .{ ra, live_vi.* });
            }
            live_vi.* = .allocating;
    ...
    const FieldPartIterator = struct {
        ...
        fn next(it: *FieldPartIterator, isel: *Select) !?struct { offset: u64, vi: Value.Index } {
            const log_fpi = std.process.hasEnvVar(isel.pt.zcu.gpa, "ZIG_LOG_FPI") catch false;
            const skip_aggregate_split = std.process.hasEnvVar(isel.pt.zcu.gpa, "ZIG_FPI_SKIP_AGGREGATE_SPLIT") catch false;
            const force_split_aggregates = std.process.hasEnvVar(isel.pt.zcu.gpa, "ZIG_FPI_SPLIT_AGGREGATES") catch false;
            // Default to NOT splitting aggregates to avoid multi-part value issues
            // Use ZIG_FPI_SPLIT_AGGREGATES=1 to enable splitting if needed
            const split_aggregates = force_split_aggregates and !skip_aggregate_split;
            const log_fpi_once = std.process.hasEnvVar(isel.pt.zcu.gpa, "ZIG_LOG_FPI_ONCE") catch false;
            if (log_fpi_once) {
                // Emit a single lightweight breadcrumb to locate long-running iterator use.
                // Shared across calls to avoid flooding logs.
                const Hit = struct {
                    fn firstHit() bool {
                        // Using a static ensures the print happens only once.
                        // Atomic order is SeqCst to avoid races across threads.
                        const std_atomic = @import("std").atomic;
                        const AtomicBool = std_atomic.Value(bool);
                        const Once = struct {
                            pub var hit = AtomicBool.init(false);
                        };
                        return !Once.hit.swap(true, .seq_cst);
                    }
                };
                if (Hit.firstHit()) {
                    std.debug.print("FPI_ONCE: field_off={d} field_size={d} next_off={d} type={f}\n", .{
                        it.field_offset,
                        it.field_size,
                        it.next_offset,
                        isel.fmtType(it.ty),
            ...
            if (log_fpi) {
                std.debug.print("FieldPartIterator: field_off={d} field_size={d} next_off={d} type={f}\n", .{
                    it.field_offset,
                    it.field_size,
                    it.next_offset,
                    isel.fmtType(it.ty),
            ...
            const addOrPanic = struct {
                fn f(a: u64, b: u64, context: []const u8, ty: ZigType, sel: *Select) u64 {
                    return std.math.add(u64, a, b) catch {
                        std.debug.panic("FieldPartIterator overflow ({s}): {d} + {d} for type {f}", .{
                            context,
                            a,
                            b,
                            sel.fmtType(ty),
            ...
            const next_part_size = std.math.sub(u64, it.field_size, next_offset) catch {
                std.debug.panic("FieldPartIterator overflow: field_size {d}, next_offset {d}", .{
                    it.field_size,
                    next_offset,
                });
            };
            if (next_part_size == 0) return null;
            var next_part_offset = addOrPanic(it.field_offset, next_offset, "field_offset+next_offset", it.ty, isel);

            const zcu = isel.pt.zcu;
            const ip = &zcu.intern_pool;
            var vi = it.vi;
            var ty = it.ty;
            const value_size = vi.size(isel);
            const ty_abi_size = ty.abiSize(zcu);
            if (value_size != ty_abi_size) {
                return isel.fail(
                    "Value.FieldPartIterator size mismatch {d} vs type {d} (field_off {d}, field_size {d}, type {f})",
                    .{ value_size, ty_abi_size, it.field_offset, it.field_size, isel.fmtType(ty) },
                );
            }
            var ty_size = ty_abi_size;
            var offset: u64 = 0;
            var size = ty_size;
            const span_end = addOrPanic(next_part_offset, next_part_size, "next_part_offset+next_part_size", ty, isel);
            if (span_end > size) {
                return isel.fail(
                    "Value.FieldPartIterator span {d} exceeds size {d} (field_off {d}, field_size {d}, type {f})",
                    .{ span_end, size, it.field_offset, it.field_size, isel.fmtType(ty) },
                );
            }
            var progress_guard: u32 = 0;
            while (next_part_offset > 0 or next_part_size < size) {
                if (progress_guard > 100_000) {
                    return isel.fail(
                        "Value.FieldPartIterator stuck (field_off {d}, field_size {d}, next_off {d}, type {f})",
                        .{ it.field_offset, it.field_size, it.next_offset, isel.fmtType(ty) },
                    );
                }
                const part_vi = vi.partAtOffset(isel, next_part_offset);
                const offset_end = addOrPanic(offset, size, "offset+size", ty, isel);
                if (part_vi != vi) {
                    vi = part_vi;
                    const part_offset, size = part_vi.position(isel);
                    const part_end = addOrPanic(part_offset, size, "part_offset+size", ty, isel);
                    // If the range is invalid, clamp next_part_offset to be within valid range
                    if (!(part_offset <= next_part_offset and part_end > next_part_offset)) {
                        // Clamp next_part_offset to part_offset if it's out of range
                        next_part_offset = if (next_part_offset < part_offset) part_offset else if (next_part_offset >= part_end) part_end -| 1 else next_part_offset;
                    }
                    offset += part_offset;
                    next_part_offset = std.math.sub(u64, next_part_offset, part_offset) catch 0;
                    continue;
                }
                progress_guard += 1;
                ...
                type_key: switch (ip.indexToKey(ty.toIntern())) {
                    ...
                    .ptr_type => |ptr_type| switch (ptr_type.flags.size) {
                        ...
                        .slice => if (!split_aggregates) {
                            // Conservatively treat the whole slice as one part to avoid
                            // issues with multi-part values in contexts expecting single parts.
                            it.next_offset = it.field_size;
                            const clamped_offset = std.math.sub(u64, next_part_offset, next_offset) catch 0;
                            return .{ .offset = clamped_offset, .vi = vi };
                        } else if (offset == 0 and size == ty_size) {
                            vi.setParts(isel, 2);
                    ...
                    .opt_type => |child_type| if (ty.optionalReprIsPayload(zcu)) continue :type_key ip.indexToKey(child_type) else {
                        ...
                        switch (child_size) {
                            ...
                            } else return isel.fail(
                                "Value.FieldPartIterator.next opt payload mismatch (offset {d}, size {d}, child_size {d}, type {f})",
                                .{ offset, size, child_size, isel.fmtType(ty) },
                            ),
                    ...
                    .array_type => |array_type| {
                        ...
                        var elem_end: u64 = 0;
                        const offset_end_array = offset_end;
                        for (0..@intCast(array_len)) |_| {
                            const elem_begin = elem_end;
                            if (elem_begin >= offset_end_array) break;
                            elem_end = addOrPanic(elem_begin, elem_size, "array elem_end", ty, isel);
                            if (elem_end <= offset) continue;
                            if (offset >= elem_begin and offset_end_array <= elem_end) {
                                ty = elem_ty;
                                ty_size = elem_size;
                                offset -= elem_begin;
                            ...
                            if (parts_len == Value.max_parts) {
                                return isel.fail("Value.FieldPartIterator.next({f}) parts overflow for array", .{isel.fmtType(ty)});
                            }
                            if (parts_len > 0) combine: {
                                const prev_part = &parts[parts_len - 1];
                                const combined_size = std.math.sub(u64, elem_end, prev_part.offset) catch {
                                    std.debug.panic("FieldPartIterator combined_size overflow: end {d}, prev_offset {d}", .{
                                        elem_end,
                                        prev_part.offset,
                                    });
                                };
                        ...
                        for (parts[0..parts_len]) |part| {
                            // Clamp underflow to 0 to handle edge cases where part.offset < offset
                            const part_off = std.math.sub(u64, part.offset, offset) catch 0;
                            const subpart_vi = vi.addPart(isel, part_off, part.size);
                    ...
                    .error_union_type => |error_union_type| {
                        if (!split_aggregates) {
                            // Conservatively treat the whole error union as one part to avoid
                            // deeply slicing aggregates that trigger overflow in pathological layouts.
                            it.next_offset = it.field_size;
                            // Clamp underflow to 0
                            const clamped_offset = std.math.sub(u64, next_part_offset, next_offset) catch 0;
                            return .{ .offset = clamped_offset, .vi = vi };
                        }
                        const min_part_log2_stride: u5 = if (size > 16) 4 else if (size > 8) 3 else 0;
                        ...
                        var parts_len: Value.PartsLen = 0;
                        var field_end: u64 = 0;
                        const offset_end_eu = offset_end;
                        for (0..2) |field_index| {
                            ...
                            if (field_begin >= offset_end_eu) break;
                            const field_size = field_ty.abiSize(zcu);
                            if (field_size == 0) continue;
                            field_end = addOrPanic(field_begin, field_size, "error-union field_end", ty, isel);
                            if (field_end <= offset) continue;
                            if (offset >= field_begin and offset_end_eu <= field_end) {
                                ty = field_ty;
                                ty_size = field_size;
                                offset -= field_begin;
                            ...
                            if (parts_len == Value.max_parts) {
                                return isel.fail("Value.FieldPartIterator.next({f}) parts overflow for error-union", .{isel.fmtType(ty)});
                            }
                            ...
                            if (parts_len > 0) combine: {
                                const prev_part = &parts[parts_len - 1];
                                const combined_size = std.math.sub(u64, field_end, prev_part.offset) catch {
                                    std.debug.panic("FieldPartIterator combined_size overflow: end {d}, prev_offset {d}", .{
                                        field_end,
                                        prev_part.offset,
                                    });
                    ...
                    .struct_type => {
                        ...
                        var parts_len: Value.PartsLen = 0;
                        var field_end: u64 = 0;
                        const offset_end_struct = offset_end;
                        var field_it = loaded_struct.iterateRuntimeOrder(ip);
                        while (field_it.next()) |field_index| {
                            const field_ty: ZigType = .fromInterned(loaded_struct.field_types.get(ip)[field_index]);
                            ...
                            if (field_begin >= offset_end_struct) break;
                            const field_size = field_ty.abiSize(zcu);
                            field_end = addOrPanic(field_begin, field_size, "struct field_end", ty, isel);
                            if (field_end <= offset) continue;
                            if (offset >= field_begin and offset_end_struct <= field_end) {
                                ty = field_ty;
                                ty_size = field_size;
                                offset -= field_begin;
                                continue :type_key ip.indexToKey(field_ty.toIntern());
                            ...
                            if (parts_len == Value.max_parts) {
                                return isel.fail("Value.FieldPartIterator.next({f}) parts overflow for struct", .{isel.fmtType(ty)});
                            }
                            const field_signedness = if (field_ty.isAbiInt(zcu)) field_signedness: {
                                const field_int_info = field_ty.intInfo(zcu);
                                break :field_signedness if (field_int_info.bits <= 16) field_int_info.signedness else null;
                    ...
                    .tuple_type => |tuple_type| {
                        ...
                        var parts_len: Value.PartsLen = 0;
                        var field_end: u64 = 0;
                        const offset_end_tuple = offset_end;
                        for (tuple_type.types.get(ip), tuple_type.values.get(ip)) |field_type, field_value| {
                            ...
                            if (field_begin >= offset_end_tuple) break;
                            const field_size = field_ty.abiSize(zcu);
                            if (field_size == 0) continue;
                            field_end = addOrPanic(field_begin, field_size, "tuple field_end", ty, isel);
                            if (field_end <= offset) continue;
                            if (offset >= field_begin and offset_end_tuple <= field_end) {
                                ty = field_ty;
                                ty_size = field_size;
                                offset -= field_begin;
                                continue :type_key ip.indexToKey(field_ty.toIntern());
                            ...
                            if (parts_len == Value.max_parts) {
                                return isel.fail("Value.FieldPartIterator.next({f}) parts overflow for tuple", .{isel.fmtType(ty)});
                            }
                            const field_is_vector = field_size <= 16 and
                                CallAbiIterator.homogeneousAggregateBaseType(zcu, field_ty.toIntern()) != null;
                            if (parts_len > 0) combine: {
                    ...
                    .union_type => {
                        ...
                        var parts_len: Value.PartsLen = 0;
                        var field_end: u64 = 0;
                        const offset_end_union = offset_end;
                        for (0..2) |field_index| {
                            ...
                            if (field_begin >= offset_end_union) break;
                            if (field_size == 0) continue;
                            field_end = addOrPanic(field_begin, field_size, "union field_end", ty, isel);
                            if (field_end <= offset) continue;
                            const field_signedness = field_signedness: switch (field) {
                                .tag => {
                                    if (offset >= field_begin and offset_end_union <= field_end) {
                            ...
                            if (parts_len == Value.max_parts) {
                                return isel.fail("Value.FieldPartIterator.next({f}) parts overflow for union", .{isel.fmtType(ty)});
                            }
                            if (parts_len > 0) combine: {
                                const prev_part = &parts[parts_len - 1];
                                const combined_size = std.math.sub(u64, field_end, prev_part.offset) catch {
            ...
            // When there's an underflow, terminate the iterator to ensure single-part return
            if (next_part_offset < next_offset) {
                it.next_offset = it.field_size;  // Terminate iterator
                return .{ .offset = 0, .vi = vi };
            }
            it.next_offset = addOrPanic(next_offset, size, "next_offset+size", ty, isel);
        ...
        fn only(it: *FieldPartIterator, isel: *Select) !?Value.Index {
            ...
            if (part) |p| {
                if (p.offset != 0) return isel.fail(
                    "Value.FieldPartIterator.only unexpected offset {d} (field_offset {d}, field_size {d}, type {f})",
                    .{ p.offset, it.field_offset, it.field_size, isel.fmtType(it.ty) },
                );
                // Return the first part's vi even if there are more parts.
                // This allows multi-part values (like slices) to be handled by the caller.
                // Mark iterator as exhausted to prevent further iteration issues.
                it.next_offset = it.field_size;
                return p.vi;
            }
            return null;
                                ...
                                .aggregate => |aggregate| switch (ip.indexToKey(aggregate.ty)) {
                                        ...
                                        switch (loaded_struct.layout) {
                                            .auto => {
                                                ...
                                                while (field_it.next()) |field_index| {
                                                    ...
                                                    const field_size = field_ty.abiSize(zcu);
                                                    const field_end = std.math.add(u64, field_offset, field_size) catch {
                                                        std.debug.panic("constant struct field_end overflow: {d}+{d}", .{ field_offset, field_size });
                                                    };
                                                    const offset_end_const = std.math.add(u64, offset, size) catch {
                                                        std.debug.panic("constant struct offset_end overflow: {d}+{d}", .{ offset, size });
                                                    };
                                                    if (offset >= field_offset and offset_end_const <= field_end) {
                                                        offset -= field_offset;
                                    ...
                                    .tuple_type => |tuple_type| {
                                        ...
                                        for (tuple_type.types.get(ip), tuple_type.values.get(ip), 0..) |field_type, field_value, field_index| {
                                            ...
                                            const field_size = field_ty.abiSize(zcu);
                                            const field_end = std.math.add(u64, field_offset, field_size) catch {
                                                std.debug.panic("constant tuple field_end overflow: {d}+{d}", .{ field_offset, field_size });
                                            };
                                            const offset_end_const = std.math.add(u64, offset, size) catch {
                                                std.debug.panic("constant tuple offset_end overflow: {d}+{d}", .{ offset, size });
                                            };
                                            if (offset >= field_offset and offset_end_const <= field_end) {
                                                offset -= field_offset;
                                                constant = switch (aggregate.storage) {
...
fn merge(
    ...
    while (live_reg_it.next()) |live_reg_entry| {
        ...
        switch (expected_vi) {
            _ => {
                switch (actual_vi.*) {
                    _ => _ = if (opts.fill_extra) {
                        if (!(try isel.fillMemory(ra))) return isel.fail("merge: fillMemory failed for {any}", .{ra});
                        if (actual_vi.* != .free) return isel.fail("merge: register {any} not free after fill", .{ra});
                    },
            ...
            .allocating => if (if (opts.fill_extra) try isel.fillMemory(ra) else try isel.fill(ra)) {
                if (actual_vi.* != .free) return isel.fail("merge: register {any} not free for allocating", .{ra});
                actual_vi.* = .allocating;
            },
            .free => if (opts.fill_extra) {
                if (!(try isel.fillMemory(ra)) or actual_vi.* != .free)
                    return isel.fail("merge: fillMemory failed to free {any}", .{ra});
            },
        }
    }
    live_reg_it = isel.live_registers.iterator();
    while (live_reg_it.next()) |live_reg_entry| {
        ...
        switch (expected_vi) {
            _ => {
                if (!(actual_vi.* == .allocating and expected_vi.register(isel) == ra)) {
                    return isel.fail("merge: unexpected state for {any} (state {any})", .{ ra, actual_vi.* });
                }
                actual_vi.* = expected_vi;
            },
            .allocating => if (actual_vi.* != .allocating)
                return isel.fail("merge: expected allocating state for {any} (got {any})", .{ ra, actual_vi.* }),
            .free => if (opts.fill_extra) if (actual_vi.* != .free)
                return isel.fail("merge: expected free state for {any} after fill", .{ ra }),
        }
    }
}
    ...
    fn finishReturn(isel: *Select) !void {
        ...
        while (live_reg_it.next()) |live_reg_entry| {
            switch (live_reg_entry.value.*) {
                _ => |live_vi| {
                    const size = live_vi.size(isel);
                    if (size == 0) {
                        // Zero-sized values don't need register handling
                        continue;
                    } else if (size <= 8) {
                        // Sizes 1-8 fit in a single register
                    } else if (size <= 16) {
                        // Sizes 9-16 need special handling
            ...
            .struct_type => {
                ...
                switch (size) {
                    ...
                    9...16 => {
                        ...
                        if (parts_len < part_sizes.len) {
                            const remaining = std.math.sub(u64, size, part_sizes[0]) catch size;
                            if (part_sizes[0] > 8 or remaining > 8) {
                                it.indirect(isel, wip_vi);
                                break :type_key;
                            }
                            part_sizes[parts_len] = remaining;
                            parts_len += 1;
                        } else if (parts_len > part_sizes.len) {
                            return isel.fail("param: struct parts overflow (len {d})", .{parts_len});
                        }
                ...
                switch (size) {
                    ...
                    9...16 => {
                        ...
                        if (parts_len < part_sizes.len) {
                            ...
                            return isel.fail("param: tuple parts overflow (len {d})", .{parts_len});
                        }
                        it.integers(isel, wip_vi, part_sizes);
                    },
                    else => it.indirect(isel, wip_vi),
