...
pub fn generate(
    ...
    const air_args = for (air_main_body, 0..) |air_inst_index, body_index| {
        ...
        const param_vi = param_vi: {
            if (arg.zir_param_index >= named_params_len) {
                ...
                if (!is_sysv) break :param_vi try param_it.nonSysvVarArg(&isel, param_ty) orelse unreachable;
            }
            break :param_vi try param_it.param(&isel, param_ty) orelse unreachable;
        };
    ...
    // Ensure we do not carry any lingering register assignments into layout/verify.
    // At this point all code has been emitted, so any leftover mappings are stale.
        ...
        while (live_reg_it.next()) |live_reg_entry| switch (live_reg_entry.value.*) {
            .allocating, .free => {},
                ...
                if (std.debug.runtime_safety) {
                    std.debug.print("scrubbing live register {s} in {f}\n", .{
