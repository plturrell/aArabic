...
export interface PopoverConfig {
    ...
    node: GraphNode;
    connectedNodes: { node: GraphNode; direction: 'incoming' | 'outgoing' }[];
}

type PopoverEventType = 'navigate' | 'expand' | 'collapse' | 'viewDetails';
...
export class NodePopover {
    ...
    private activeTab: 'details' | 'links' = 'details';

    // Callbacks
    private onNavigateCallback: ((nodeId: string) => void) | null = null;
    private onExpandCallback: ((nodeId: string) => void) | null = null;
    private onCollapseCallback: ((nodeId: string) => void) | null = null;
    private onViewDetailsCallback: ((nodeId: string) => void) | null = null;
    ...
    public show(config: PopoverConfig): void {
    ...
    public on(event: PopoverEventType, callback: (nodeId: string) => void): void {
        switch (event) {
            case 'navigate':
    ...
    private createElement(config: PopoverConfig): HTMLElement {
    ...
    private buildPopoverHTML(config: PopoverConfig): string {
    ...
    private buildDetailsTab(node: GraphNode): string {
    ...
    private buildLinksTab(connectedNodes: { node: GraphNode; direction: 'incoming' | 'outgoing' }[]): string {
        const incoming = connectedNodes.filter(c => c.direction === 'incoming');
        const outgoing = connectedNodes.filter(c => c.direction === 'outgoing');

        const buildLinkList = (nodes: { node: GraphNode; direction: 'incoming' | 'outgoing' }[], direction: string): string => {
            if (nodes.length === 0) {
    ...
    private setupPopoverEvents(popover: HTMLElement, config: PopoverConfig): void {
        // Close button
        ...
        tabs.forEach(tab => {
            tab.addEventListener('click', (e) => {
                ...
                const tabName = target.getAttribute('data-tab') as 'details' | 'links';
                ...
                contents.forEach(content => {
                    const contentName = (content as HTMLElement).getAttribute('data-content');
    ...
    private handleClickOutside(e: MouseEvent): void {
        if (!this.popoverElement || !this.visible) return;
