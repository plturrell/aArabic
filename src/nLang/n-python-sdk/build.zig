...
pub fn build(b: *std.Build) void {
    ...
    const i18n_module = b.createModule(.{
        .root_source_file = b.path("compiler/i18n/i18n.zig"),
    });

    // Plurals module
    const plurals_module = b.createModule(.{
        .root_source_file = b.path("compiler/i18n/plurals.zig"),
    });

    // Locale detection module
    const locale_detect_module = b.createModule(.{
        .root_source_file = b.path("compiler/i18n/locale_detect.zig"),
    });
    locale_detect_module.addImport("i18n", i18n_module);

    // DateTime formatting module
    const datetime_module = b.createModule(.{
        .root_source_file = b.path("compiler/i18n/datetime.zig"),
    });
    ...
    // ========================================================================
    // CLI Tool Executable - ENABLED for Production Use
    // ========================================================================
    ...
    // Add compiler driver imports for CLI
    cli_exe.root_module.addImport("driver", driver_module);
    cli_exe.root_module.addImport("lexer", lexer_module);
    cli_exe.root_module.addImport("parser", parser_module);
    cli_exe.root_module.addImport("ast", ast_module);

    // Add i18n imports for CLI
    cli_exe.root_module.addImport("i18n", i18n_module);
    cli_exe.root_module.addImport("locale_detect", locale_detect_module);
    cli_exe.root_module.addImport("plurals", plurals_module);
    cli_exe.root_module.addImport("datetime", datetime_module);
