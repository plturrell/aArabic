...
type ToolbarEventType = 'search' | 'zoomIn' | 'zoomOut' | 'fitToView' | 'fullscreen' | 'legendToggle';
...
export class NetworkGraphToolbar {
    ...
    // State
    private isLegendVisible: boolean = false;
    ...
    private onFitToView: (() => void) | null = null;
    private onFullscreen: ((enabled: boolean) => void) | null = null;
    private onLegendToggle: ((visible: boolean) => void) | null = null;

    // Default config
    private static readonly DEFAULT_CONFIG: ToolbarConfig = {
        title: 'Network Graph',
        showSearch: true,
        showLegend: true,
    ...
    constructor(container: HTMLElement, config: Partial<ToolbarConfig> = {}) {
    ...
    private createLegendButton(): HTMLButtonElement {
    ...
    private setupEventListeners(): void {
        ...
        if (this.fitButton) {
            this.fitButton.addEventListener('click', () => {
                if (this.onFitToView) this.onFitToView();
            });
        }
    ...
    private updateButtonActiveState(button: HTMLButtonElement, active: boolean): void {
        if (active) {
    ...
    public setZoomLevel(percent: number): void {
    ...
    public setSearchSuggestions(suggestions: string[]): void {
    ...
    public on(event: ToolbarEventType, callback: Function): void {
        switch (event) {
            case 'search':
                this.onSearch = callback as (query: string) => void;
                break;
            case 'zoomIn':
                this.onZoomIn = callback as () => void;
                break;
            case 'zoomOut':
                this.onZoomOut = callback as () => void;
                break;
            case 'fitToView':
                this.onFitToView = callback as () => void;
                break;
            case 'fullscreen':
                this.onFullscreen = callback as (enabled: boolean) => void;
                break;
            case 'legendToggle':
                this.onLegendToggle = callback as (visible: boolean) => void;
                break;
        }
    }
