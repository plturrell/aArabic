...
# Run parser over a subset of Lean4 tests and store syntax output.
...
if ! command -v mojo &> /dev/null; then
    echo "mojo not found; skipping conformance parser run"
    exit 0
fi
...
out_dir="$PROJECT_ROOT/tmp/conformance/parser"
mkdir -p "$out_dir"
