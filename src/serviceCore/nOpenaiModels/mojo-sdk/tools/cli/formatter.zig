...
pub fn formatFiles(allocator: Allocator, options: FormatOptions) !void {
    var files_to_format = std.ArrayList([]const u8).init(allocator);
    defer {
        for (files_to_format.items) |file| {
            allocator.free(file);
        }
        files_to_format.deinit();
    }
    ...
    for (options.files) |file_or_dir| {
        if (options.recursive) {
            ...
            try files_to_format.append(file);
        }
    }
...
fn formatAST(allocator: Allocator, ast: *anyopaque) ![]const u8 {
    ...
    try result.appendSlice("fn main() {\n");
    try result.appendSlice("    let x = 42\n");
    try result.appendSlice("    print(x)\n");
