...
struct InferenceEngine:
    ...
    fn generate(
        ...
        # Safe copy: build string by copying each byte before freeing buffer
        let actual_length = Int(length)
        var result = String()
        for i in range(actual_length):
            let byte_val = result_buffer[i]
            result += chr(Int(byte_val))

        # Now safe to free the buffer after copying
        result_buffer.free()
        return result
    ...
    fn get_info(self) raises -> String:
        ...
        var info = String()
        for i in range(actual_length):
            let byte_val = buffer[i]
            info += chr(Int(byte_val))
    ...
    fn get_library_path(self) -> String:
        """Get the path of the loaded inference library."""
...
fn generate_with_model(
    ...
    """
    Generate text using a fresh engine instance.
    This avoids global state (not supported in this Mojo version).
    """
