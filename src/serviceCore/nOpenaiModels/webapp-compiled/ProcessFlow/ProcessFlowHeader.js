...
export class ProcessFlowHeader {
    constructor(container) {
        this.lanes = [];
        this.laneWidth = 160;
        this.laneOffset = PROCESS_FLOW_LAYOUT.spacing.laneHeader + PROCESS_FLOW_LAYOUT.spacing.leftMargin;
        // Elements per lane
        this.laneElements = new Map();
        // Tooltip element
        this.tooltip = null;
        // Event callbacks
    ...
    createArcPath(cx, cy, radius, startAngle, endAngle, color) {
    ...
    // Event Handlers
    // ========================================================================
    handleLaneClick(event, laneId) {
        event.stopPropagation();
        if (this.onLaneClick) {
            this.onLaneClick(laneId);
        }
    }
    handleLaneHover(event, lane, isEnter) {
    ...
    showTooltip(event, lane) {
        ...
        const lines = [`<strong>${lane.title}</strong>`];
...
//# sourceMappingURL=ProcessFlowHeader.js.map