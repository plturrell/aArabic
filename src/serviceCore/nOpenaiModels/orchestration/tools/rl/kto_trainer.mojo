...
struct KTOTrainer:
    ...
    fn __init__(
        ...
        # Creates a deep copy of the policy to serve as stable baseline
        self.reference_policy = self._clone_policy_for_reference(policy)
    ...
    @staticmethod
    fn _clone_policy_for_reference(policy: KTOPolicy) -> KTOPolicy:
        """
        Create a deep copy of the policy for use as reference

        The reference policy is used for KL divergence computation
        and is updated via EMA to provide a stable baseline.
        """
        var cloned = KTOPolicy(
            registry=policy.registry,
            d_model=policy.transformer.d_model,
            n_heads=policy.transformer.n_heads,
            n_layers=policy.transformer.n_layers,
            mhc_stability_weight=policy.mhc_stability_weight
        )

        # Copy hyperparameters
        cloned.lambda_loss_aversion = policy.lambda_loss_aversion
        cloned.reference_policy_ema = policy.reference_policy_ema
        cloned.temperature = policy.temperature

        # Copy mHC config
        cloned.mhc_config = policy.mhc_config

        # Copy stability tracking state
        cloned.total_updates = policy.total_updates
        cloned.stable_updates = policy.stable_updates
    ...
    fn _update_reference_policy(inout self):
        ...
        This implements a soft update where the reference policy slowly tracks
        the current policy, providing a stable baseline for KL divergence.
        """
        let alpha = self.reference_ema_alpha

        # Update transformer parameters
        self._ema_update_transformer(alpha)

        # Update action head parameters
        self._ema_update_action_head(alpha)

        # Update value head parameters
        self._ema_update_value_head(alpha)

        # Update KTO hyperparameters (typically static, but could be adaptive)
        self.reference_policy.lambda_loss_aversion = (
            alpha * self.reference_policy.lambda_loss_aversion +
            (1.0 - alpha) * self.policy.lambda_loss_aversion
        )
        self.reference_policy.temperature = (
            alpha * self.reference_policy.temperature +
            (1.0 - alpha) * self.policy.temperature
        )

    fn _ema_update_transformer(inout self, alpha: Float32):
        """
        EMA update for transformer model parameters

        Updates d_model, n_heads, n_layers, d_ff dimensions
        Note: In a full implementation, this would update actual weight matrices
        """
        # Transformer architecture parameters (for placeholder implementation)
        # In production: iterate over transformer.parameters() and apply EMA
        let ref_transformer = self.reference_policy.transformer
        let cur_transformer = self.policy.transformer

        # EMA on transformer dimensions (these are typically fixed, but shown for completeness)
        # Real implementation would do: weight_ref = alpha * weight_ref + (1-alpha) * weight_cur
        self.reference_policy.transformer.d_model = int(
            alpha * Float32(ref_transformer.d_model) +
            (1.0 - alpha) * Float32(cur_transformer.d_model)
        )

    fn _ema_update_action_head(inout self, alpha: Float32):
        """
        EMA update for action head parameters

        Updates linear layer weights mapping hidden states to action logits
        """
        # In production: EMA on actual weight matrices
        # action_head.weight = alpha * action_head_ref.weight + (1-alpha) * action_head.weight
        let ref_head = self.reference_policy.action_head
        let cur_head = self.policy.action_head

        self.reference_policy.action_head.input_dim = int(
            alpha * Float32(ref_head.input_dim) +
            (1.0 - alpha) * Float32(cur_head.input_dim)
        )
        self.reference_policy.action_head.num_actions = int(
            alpha * Float32(ref_head.num_actions) +
            (1.0 - alpha) * Float32(cur_head.num_actions)
        )

    fn _ema_update_value_head(inout self, alpha: Float32):
        """
        EMA update for value head parameters

        Updates linear layer weights mapping hidden states to value estimate
        """
        let ref_head = self.reference_policy.value_head
        let cur_head = self.policy.value_head

        self.reference_policy.value_head.input_dim = int(
            alpha * Float32(ref_head.input_dim) +
            (1.0 - alpha) * Float32(cur_head.input_dim)
        )

    fn clone_policy(self) -> KTOPolicy:
        """
        Create a deep copy of the current policy for reference

        Used for initializing reference policy or checkpointing
        """
        var cloned = KTOPolicy(
            registry=self.policy.registry,
            d_model=self.policy.transformer.d_model,
            n_heads=self.policy.transformer.n_heads,
            n_layers=self.policy.transformer.n_layers,
            mhc_stability_weight=self.policy.mhc_stability_weight
        )

        # Copy hyperparameters
        cloned.lambda_loss_aversion = self.policy.lambda_loss_aversion
        cloned.reference_policy_ema = self.policy.reference_policy_ema
        cloned.temperature = self.policy.temperature

        # Copy mHC config
        cloned.mhc_config = self.policy.mhc_config

        return cloned

    fn hard_update_reference(inout self):
        """
        Hard update: copy current policy to reference entirely

        Useful for periodic full synchronization
        """
        self.reference_policy = self.clone_policy()

    fn get_ema_coefficient(self) -> Float32:
        """Get current EMA coefficient"""
        return self.reference_ema_alpha

    fn set_ema_coefficient(inout self, alpha: Float32):
        """
        Set EMA coefficient

        Args:
            alpha: New EMA coefficient in [0, 1]
                   Higher values = slower reference updates
                   0.99 = reference updates very slowly (stable baseline)
                   0.9 = reference updates faster (more responsive)
        """
        self.reference_ema_alpha = max(0.0, min(1.0, alpha))
