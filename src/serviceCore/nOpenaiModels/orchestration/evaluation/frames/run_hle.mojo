"""
Mojo replacement for run_hle.py.
Provides CLI commands and full orchestration loop for HLE evaluation jobs.
"""
...
fn write_model_config(serve_ips1: List[String], serve_ips2: List[String], ckpt_dir: String) raises:
    ...
    for i in range(len(serve_ips1)):
        if i > 0: config += ","
        config += '{"ip_addr": "' + serve_ips1[i] + '", "port": "1404"}'
    ...
    write_file("model_configs/serve2.json", config)
...
fn orchestrate(ckpt_dir: String) raises:
    ...
    print("Starting orchestration for checkpoint:", ckpt_dir)
    while True:
        ...
        for job in jobs:
            if job.name not in serve_collections and job.name.startswith("av"):
                job_control.cancel(job.id)
        ...
        for repeat in range(SERVE_REPEAT):
            serve_ips1.append(read_serve_ip("run_" + String(repeat)))
        var serve_ips2 = List[String]()
        for repeat in range(SERVE_REPEAT):
            serve_ips2.append(read_serve_ip("av_2" + String(repeat)))
        if serve_ips1 != prev_ips1 or serve_ips2 != prev_ips2:
