...
pub fn generateDocs(allocator: Allocator, options: DocOptions) !void {
    ...
    const source_files = try discoverSourceFiles(allocator, options.input_dir);
    defer {
    ...
    var doc_items = std.ArrayList(DocItem).init(allocator);
    defer {
...
fn generateHTML(allocator: Allocator, doc_items: *std.ArrayList(DocItem), options: DocOptions) !void {
    ...
    // Write to file
    try std.fs.cwd().writeFile(index_path, html.items);
}
...
fn generateMarkdown(allocator: Allocator, doc_items: *std.ArrayList(DocItem), options: DocOptions) !void {
    ...
    var md = std.ArrayList(u8).init(allocator);
    defer md.deinit();
    ...
    // Write to file
    try std.fs.cwd().writeFile(index_path, md.items);
}
