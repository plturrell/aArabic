...
"""
HLE Retrieval Server - Pure Mojo Implementation

This module provides an HTTP server for HLE (Humanity's Last Exam) retrieval using:
- Zig-based HTTP server via FFI
- BM25 text search (pure Mojo, no FAISS/transformers)
- Pure Mojo data handling

Usage:
    mojo run retrieval_hle.mojo --port 1401 --corpus ./data/eval.jsonl
"""
...
struct RetrievalConfig:
    """Configuration for the HLE retrieval server."""
    ...
    var example_id_file: String
    var new_cache_dir: String
    var retrieval_query_max_length: Int

    fn __init__(inout self):
        self.retrieval_method = "bm25"
        self.retrieval_topk = 10
        self.index_path = "./index/hle"
        self.corpus_path = "./data/eval.jsonl"
        self.faiss_gpu = False  # GPU not available in pure Mojo
        self.retrieval_batch_size = 128
        self.example_id_file = "examples.json"
        self.new_cache_dir = "cache/hle"
        self.retrieval_query_max_length = 32768
...
struct Corpus:
    """In-memory corpus for HLE retrieval."""
...
fn parse_json_int_list(json_str: String) -> List[Int]:
    """Parse a JSON list of integers."""
    var result = List[Int]()
    var i = 0
    var current = String("")
...
var g_example_ids: Dict[String, List[Int]] = Dict[String, List[Int]]()


fn load_example_ids(path: String) raises:
    """Load example IDs from JSON file."""
    if not file_exists(path):
        print("Warning: Example ID file not found:", path)
        return

    var content = read_file(path)
    # Parse JSON object with string keys and list values
    # Format: {"eid1": [1,2,3], "eid2": [4,5,6], ...}
    var i = 0
    while i < len(content):
        # Find key
        var key_start = content.find('"', i)
        if key_start == -1:
            break
        var key_end = content.find('"', key_start + 1)
        if key_end == -1:
            break
        var key = content[key_start + 1:key_end]

        # Find value list
        var list_start = content.find('[', key_end)
        if list_start == -1:
            break
        var list_end = content.find(']', list_start)
        if list_end == -1:
            break

        var list_str = content[list_start:list_end + 1]
        g_example_ids[key] = parse_json_int_list(list_str)
        i = list_end + 1

    print("Loaded example IDs for", len(g_example_ids), "examples")


fn handle_retrieve_request(query: String, topk: Int, return_scores: Bool, eid: String) -> String:
    """Handle a retrieve request and return JSON response."""
    var results = g_index.search(query, g_corpus, topk * 10)  # Get more for filtering

    # Filter by example IDs if eid provided
    var filtered_results = List[Tuple[Int, Float64]]()
    if eid != "" and eid in g_example_ids:
        var valid_ids = g_example_ids[eid]
        for r in results:
            var doc_idx = r[][0]
            var doc = g_corpus.documents[doc_idx]
            try:
                var doc_id_int = atol(doc.id)
                for valid_id in valid_ids:
                    if doc_id_int == valid_id[]:
    ...
    for i in range(len(results)):
        ...
        # Filter by content length and score (mimicking Python behavior)
        if len(doc.content) <= 100 or score <= 0.1:
            continue

        if count > 0:
            response += ","
        response += '{"document": {"id": "' + doc.id + '", "content": "' + escape_json(doc.content) + '"}'
        if return_scores:
            response += ', "score": ' + String(score)
        response += '}'
        count += 1
...
fn handle_health_request() -> String:
    ...
    return '{"status": "healthy", "corpus_size": ' + String(len(g_corpus)) + ', "index_terms": ' + String(len(g_index.term_doc_freq)) + '}'
...
fn parse_corpus_path_from_args() -> String:
    ...
    var path = String("./data/eval.jsonl")
    var args = argv()
    var i = 1
    while i < len(args):
...
fn parse_example_id_file_from_args() -> String:
    """Parse --example_id_file argument from command line arguments."""
    var path = String("examples.json")
    var args = argv()
    var i = 1
    while i < len(args):
        if args[i] == "--example_id_file" and i + 1 < len(args):
            path = args[i + 1]
            break
        i += 1
    return path
...
fn parse_cache_dir_from_args() -> String:
    """Parse --new_cache_dir argument from command line arguments."""
    var path = String("cache/hle")
    var args = argv()
    var i = 1
    while i < len(args):
        if args[i] == "--new_cache_dir" and i + 1 < len(args):
            path = args[i + 1]
            break
        i += 1
    return path
...
fn main() raises:
    """Main entry point for the HLE retrieval server."""
    print("🔍 HLE Retrieval Server (Pure Mojo)")
    print("====================================")

    var port = parse_port_from_args()
    var corpus_path = parse_corpus_path_from_args()
    var example_id_file = parse_example_id_file_from_args()
    var cache_dir = parse_cache_dir_from_args()

    # Update config
    g_config.corpus_path = corpus_path
    g_config.example_id_file = example_id_file
    g_config.new_cache_dir = cache_dir
    ...
    print("Loading example IDs from:", example_id_file)
    load_example_ids(example_id_file)

    print("Starting server on port", port)
    print("Endpoints:")
    print("  GET  /health    - Health check")
    print("  POST /retrieve  - Retrieve documents")
    ...
    #   zig build-exe shared/http/server.zig -o hle_server
    #   ./hle_server --port 1401

    # Demo search
    print("\n📝 Demo search for 'quantum physics':")
    var demo_results = handle_retrieve_request("quantum physics", 5, True, "")
    print(demo_results)

    print("\n✅ HLE retrieval server ready. Use Zig HTTP server for production.")
