...
const BenchmarkScoring = @import("benchmark_scoring.zig").BenchmarkScoring;
const MultiCategory = @import("multi_category.zig");
const MultiCategoryRegistry = MultiCategory.MultiCategoryRegistry;
const MultiCategoryModel = MultiCategory.MultiCategoryModel;
...
pub const ModelSelector = struct {
    ...
    benchmark_scoring: ?*BenchmarkScoring,
    multi_category_registry: ?*MultiCategoryRegistry,
    ...
    pub fn init(
        ...
        selector.* = .{
            ...
            .benchmark_scoring = null,
            .multi_category_registry = null,
            .gpu_monitor = null,
        };
    ...
    pub fn deinit(self: *ModelSelector) void {
        ...
        if (self.multi_category_registry) |registry| {
            registry.deinit();
        }
    ...
    fn calculateScore(
        ...
        // Add benchmark-based scoring if available
        if (self.benchmark_scoring) |scorer| {
            ...
            if (model.benchmarks) |bm| {
                ...
                var it = bm.iterator();
                while (it.next()) |entry| {
                    try benchmark_list.append(.{
                        .name = entry.key_ptr.*,
                        .score = @floatCast(entry.value_ptr.score),
                        .metric = "accuracy", // Default metric
                    });
                }
                
                const benchmark_score = scorer.scoreModel(benchmark_list.items, task_category);
                score += benchmark_score; // Add 0-50 points from benchmarks
            }
        }
    ...
    /// Enable benchmark-based scoring
    pub fn enableBenchmarkScoring(self: *ModelSelector) !void {
        if (self.benchmark_scoring == null) {
            self.benchmark_scoring = try BenchmarkScoring.init(self.allocator);
        }
    }
    
    /// Enable multi-category support
    pub fn enableMultiCategory(self: *ModelSelector) !void {
        if (self.multi_category_registry == null) {
            self.multi_category_registry = try MultiCategoryRegistry.init(self.allocator);
        }
    }
    
    /// Build multi-category registry from loaded models
    pub fn buildMultiCategoryRegistry(self: *ModelSelector) !void {
        try self.enableMultiCategory();
        
        const registry = self.multi_category_registry.?;
        
        // Convert each model to multi-category
        for (self.models.items) |model| {
            if (model.orchestration_categories.len == 0) continue;
            
            const mc_model = try MultiCategoryModel.init(self.allocator, model.name);
            
            // Add each category with computed scores
            for (model.orchestration_categories) |category| {
                const base_score: f32 = 50.0;
                var benchmark_score: f32 = 0.0;
                ...
                if (self.benchmark_scoring) |scorer| {
                    if (model.benchmarks) |bm| {
                        ...
                        benchmark_score = scorer.scoreModel(benchmark_list.items, category);
                    }
                }
                ...
                try mc_model.addCategory(category, base_score, benchmark_score, confidence);
            }
            
            try registry.registerModel(mc_model);
        }
    }
    
    /// Select model with multi-category support
    pub fn selectModelMultiCategory(
        self: *ModelSelector,
        task_category: []const u8,
        constraints: SelectionConstraints,
    ) !SelectionResult {
        // Ensure multi-category registry exists
        if (self.multi_category_registry == null) {
            try self.buildMultiCategoryRegistry();
        }
        
        const registry = self.multi_category_registry.?;
        
        // Get all models supporting this category
        const candidate_models = try registry.getModelsForCategory(self.allocator, task_category);
        defer {
            for (candidate_models) |m| self.allocator.free(m);
            self.allocator.free(candidate_models);
        }
        ...
        for (candidate_models) |model_name| {
            const model = self.findModel(model_name) orelse continue;
            ...
            // Get multi-category score
            const mc_model = registry.getModel(model_name).?;
            const mc_score = mc_model.scoreForCategory(task_category) orelse continue;
        ...
        var score_buf: [32]u8 = undefined;
        const score_str = try std.fmt.bufPrint(&score_buf, "{d:.2}", .{best_score});
        try reason.appendSlice(score_str);
        
        return SelectionResult{
            .model = best_model.?,
            .score = best_score,
            .reason = try reason.toOwnedSlice(),
        };
    }
    
    /// Enable GPU monitoring
    pub fn enableGPUMonitoring(self: *ModelSelector, update_interval_ms: u32) !void {
        if (self.gpu_monitor == null) {
            self.gpu_monitor = try GPUMonitor.init(self.allocator, update_interval_ms);
        }
    }
    
    /// Select model with GPU-aware routing
    pub fn selectModelGPUAware(
        self: *ModelSelector,
        task_category: []const u8,
        constraints: SelectionConstraints,
    ) !SelectionResult {
        // Enable GPU monitoring if not already enabled
        if (self.gpu_monitor == null) {
            try self.enableGPUMonitoring(5000); // 5 second refresh
        }
        
        const monitor = self.gpu_monitor.?;
        
        // Use multi-category selection as base
        if (self.multi_category_registry == null) {
            try self.buildMultiCategoryRegistry();
        }
        ...
        // Score each candidate with GPU awareness
        for (candidate_models) |model_name| {
            const model = self.findModel(model_name) orelse continue;
            
            // Check GPU availability
            const model_mem = try model.getGpuMemoryMB();
            const gpu_selection = monitor.selectBestGPU(model_mem) catch continue;
            ...
            // Adjust score based on GPU load (lower load = higher score)
            const gpu_load_factor = 1.0 - gpu_selection.load_score;
            const adjusted_score = mc_score * gpu_load_factor;
            
            if (adjusted_score > best_score) {
                best_score = adjusted_score;
                best_model = model;
                best_gpu_id = gpu_selection.device_id;
            }
        }
        ...
        const gpu_str = try std.fmt.bufPrint(&gpu_buf, "{d}", .{best_gpu_id});
        try reason.appendSlice(gpu_str);
        
        try reason.appendSlice(" with score ");
        var score_buf: [32]u8 = undefined;
        const score_str = try std.fmt.bufPrint(&score_buf, "{d:.2}", .{best_score});
        try reason.appendSlice(score_str);
