...
from inference.bridge.inference_api import shared_get_info, shared_is_model_loaded
...
fn handle_health() -> UnsafePointer[UInt8, MutExternalOrigin]:
    ...
    var model_loaded = shared_is_model_loaded()
    var model_info = ""
    if model_loaded:
        try:
            model_info = shared_get_info()
        except:
            model_info = ""
...
fn main() raises:
    ...
    try:
        zig_lib = OwnedDLHandle("./libzig_http_shimmy.dylib")
    except:
        try:
            zig_lib = OwnedDLHandle("./libzig_http_shimmy.so")
        except:
    ...
    var host_ptr = string_to_c_ptr(host)
    var result = external_call["zig_shimmy_serve_simple", Int32](
        zig_lib,
        port,
        host_ptr,
        handle_http_request
    ...
    if result != 0:
        print("❌ Zig HTTP server exited with error")
        return
