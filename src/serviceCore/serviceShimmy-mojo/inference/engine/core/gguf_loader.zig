...
pub const GGUFModel = struct {
    ...
    pub fn load(allocator: mem.Allocator, path: []const u8) !GGUFModel {
        ...
        var header: GGUFHeader = undefined;
        const bytes_read = try file.read(mem.asBytes(&header));

        if (bytes_read != @sizeOf(GGUFHeader)) {
...
fn parseMetadataValue(
    ...
    switch (value_type) {
        ...
        .String => {
            ...
            // Skip string content for now (can read if needed)
            try file.seekBy(@intCast(str_len));

            // Check for architecture
            if (mem.eql(u8, key, "general.architecture")) {
                // Would need to read string to detect architecture
                // For now, assume Llama
