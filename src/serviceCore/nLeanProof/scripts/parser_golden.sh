...
# Golden parser tests (Mojo CLI).

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
GOLDEN_DIR="$PROJECT_ROOT/tests/parser/golden"
PARSER_CLI="$PROJECT_ROOT/core/parser/parser_cli.mojo"
...
if ! command -v mojo &> /dev/null; then
    echo "mojo not found; skipping parser golden tests"
    exit 0
fi
...
if [[ ! -f "$PARSER_CLI" ]]; then
    echo "Parser CLI missing: $PARSER_CLI" >&2
    exit 1
fi
...
for input in "$GOLDEN_DIR"/*.lean; do
    ...
    expected="$GOLDEN_DIR/$base.syntax"
    if [[ ! -f "$expected" ]]; then
        echo "Missing expected syntax: $expected" >&2
        fail=1
        continue
    fi

    text="$(cat "$input")"
    output="$(run_mojo run "$PARSER_CLI" --text "$text")"
...
if [[ $found -eq 0 ]]; then
    echo "No golden parser tests found in $GOLDEN_DIR"
    exit 0
fi

if [[ $fail -ne 0 ]]; then
    echo "Golden parser tests failed" >&2
    exit 1
fi

echo "Golden parser tests passed"
